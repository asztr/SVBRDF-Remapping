 ...
 */
class pbr : public BSDF {
public:
	pbr(const Properties &props) : BSDF(props) {
		ref<FileResolver> fResolver = Thread::getThread()->getFileResolver();

		m_specularReflectance = new ConstantSpectrumTexture(
			props.getSpectrum("specularReflectance", Spectrum(1.0f)));

		m_eta = new ConstantSpectrumTexture(
			props.getSpectrum("eta", Spectrum(1.0f)));

		m_k = new ConstantSpectrumTexture(
			props.getSpectrum("k", Spectrum(1.0f)));
	...
	pbr(Stream *stream, InstanceManager *manager)
	 : BSDF(stream, manager) {
...
public:
	pbrShader(Renderer *renderer, const Texture *specularReflectance,
...
MTS_EXPORT_PLUGIN(pbr, "Rough conductor BRDF");
MTS_NAMESPACE_END
